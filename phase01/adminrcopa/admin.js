...
function prepararEdicion(idPedido) {
  ...
  $('#edit_id_visual').val(reg.id_pedido)
  $('#edit_n1').val(reg.integrante_1)
  $('#edit_dni').val(reg.dni ? String(reg.dni).replace(/^'/, '') : '')
  $('#edit_celular').val(
    reg.celular ? String(reg.celular).replace(/^'/, '') : '',
  )
  $('#edit_seccion').val(reg.seccion || '')
...
function calcularPromedio(val4, val5) {
  const str4 = val4 !== null && val4 !== undefined ? val4.toString().trim() : ''
  const str5 = val5 !== null && val5 !== undefined ? val5.toString().trim() : ''
...
async function reenviarEmailFase2(id) {
  ...
  // Determinar texto del equipo para el correo
  const detalleEquipo =
    !reg.grupo || reg.grupo === 'Individual'
      ? `<p style="margin: 0 0 10px 0;"><strong>Modalidad:</strong> Individual</p>`
      : `<p style="margin: 0 0 10px 0;"><strong>Equipo Asignado:</strong> ${reg.grupo}</p>`

  // Construir previsualización exacta del correo
  const previewHtml = `
    <div style="text-align: left; margin-bottom: 15px;">
      <span class="badge bg-light text-dark border shadow-sm px-3 py-2" style="font-size: 0.85rem;">
        <i class="fas fa-paper-plane text-primary me-1"></i> Destinatario: <strong>${reg.correo}</strong>
      </span>
    </div>
    <div style="font-family: Arial, sans-serif; color: #2E004F; text-align: left; font-size: 0.85rem; border: 1px solid #E2E8F0; padding: 15px; border-radius: 10px; background-color: #FFFFFF; max-height: 45vh; overflow-y: auto; box-shadow: inset 0 0 10px rgba(0,0,0,0.02);">
      <div style="text-align: center; margin-bottom: 20px;">
        <img src="https://d30mzt1bxg5llt.cloudfront.net/public/uploads/images/LOGO-NEUMANN.png" alt="Logo" style="max-width: 140px; height: auto;">
      </div>
      <h3 style="color: #4B0082; text-align: center; font-size: 1.1rem; margin-top:0;">Bienvenido(a)</h3>
      <p>Hola <strong>${reg.integrante_1}</strong>, tu registro ha sido procesado. A continuación, los datos de tu proyecto y la plantilla oficial:</p>
      
      <div style="background-color: #F8F9FA; padding: 12px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #4B0082;">
        <p style="margin: 0 0 8px 0;"><strong>ID de Equipo:</strong> <span style="color: #9D50BB; font-weight: bold;">${reg.id_pedido}</span></p>
        <p style="margin: 0 0 8px 0;"><strong>Programa:</strong> ${reg.programa || '---'}</p>
        ${detalleEquipo}
      </div>

      <p style="text-align: center; margin: 20px 0;">
        <span style="background-color: #4B0082; color: #FFFFFF; padding: 10px 20px; border-radius: 5px; font-weight: bold; display: inline-block; cursor: not-allowed; opacity: 0.9;">
          Descargar Plantilla (Botón Simulado)
        </span>
      </p>

      <div style="margin-top: 20px; font-size: 0.8rem; color: #4A4A5A; background-color: #F8F9FA; padding: 12px; border-radius: 8px;">
        <strong style="color: #6A0DAD; display:block; margin-bottom: 8px;">Pautas Generales:</strong>
        <ul style="padding-left: 20px; margin: 0;">
          <li style="margin-bottom: 4px;">El botón anterior descargará automáticamente el archivo en formato Word.</li>
          <li style="margin-bottom: 4px;">Mantener la estructura y numeración.</li>
          <li style="margin-bottom: 4px;">El documento final debe cargarse en formato PDF.</li>
        </ul>
      </div>
    </div>
  `
...
async function enviarTodosFase2() {
  ...
  const listaEstudiantes = candidatos
    .map(
      (c) =>
        `<li style="margin-bottom: 6px;"><strong>${c.integrante_1}</strong><br><span style="font-size: 0.75rem; color: #6A0DAD;"><i class="fas fa-envelope me-1"></i>${c.correo}</span></li>`,
    )
    .join('')
  const total = candidatos.length
  const htmlLista = `
    <div class="text-start">
      <p class="mb-2">Se enviará la <b>Plantilla de Estructura</b> a <b>${total}</b> destinatarios:</p>
      <div style="max-height: 250px; overflow-y: auto; border: 1px solid #E2E8F0; border-radius: 8px; padding: 12px; background-color: #F8F9FA; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">
        <ul style="font-size: 0.85rem; color: #4A4A5A; margin: 0; padding-left: 20px;">
          ${listaEstudiantes}
        </ul>
      </div>
    </div>`
...
async function reenviarEmailFase4(id) {
  ...
  const previewHtml = `
    ...
    <div style="font-family: Arial, sans-serif; color: #2E004F; text-align: left; font-size: 0.85rem; border: 1px solid #E2E8F0; padding: 20px; border-radius: 12px; background-color: #FFFFFF; max-height: 45vh; overflow-y: auto;">
      <div style="text-align: center; margin-bottom: 20px;">
        <img src="https://d30mzt1bxg5llt.cloudfront.net/public/uploads/images/LOGO-NEUMANN.png" alt="Logo" style="max-width: 140px;">
        <h3 style="color: #4B0082; margin: 10px 0 0 0; font-size: 1.1rem;">Resultado de Evaluación Técnica</h3>
      </div>
      <p>Estimado/a <strong>${reg.integrante_1}</strong>,</p>
      <p>Se ha concluido la revisión técnica de tu proyecto: <br><em style="color: #6A0DAD;">"${reg.tema}"</em>.</p>
      <div style="background-color: #F8F9FA; border-radius: 10px; padding: 15px; margin: 20px 0; border: 1px solid #6A0DAD; text-align: center;">
        <p style="margin-bottom: 12px;">Accede al detalle de tu calificación y observaciones aquí:</p>
        <span style="background-color: #4B0082; color: #FFFFFF; padding: 10px 20px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; opacity: 0.9;">Ver Informe de Evaluación</span>
        <p style="margin-top: 15px; font-size: 0.75rem; color: #8b7d93;">ID de Expediente: <strong>${reg.id_pedido}</strong></p>
      </div>
      <p style="font-size: 0.75rem; color: #4A4A5A;">Si el informe indica <b>No Conforme</b>, realiza los ajustes indicados y vuelve a subir el archivo.</p>
    </div>`

  const confirm = await Swal.fire({
    title: '¿Enviar Informe de Fase 04?',
    html: previewHtml,
    icon: 'info',
    width: '600px',
    showCancelButton: true,
...
async function enviarTodosFase4() {
  ...
  const htmlLista = `
    <div class="text-start">
      <p class="mb-2">Se enviarán los <b>Informes de Evaluación</b> a <b>${total}</b> destinatarios:</p>
      <div style="max-height: 250px; overflow-y: auto; border: 1px solid #E2E8F0; border-radius: 8px; padding: 12px; background-color: #F8F9FA; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">
        <ul style="font-size: 0.85rem; color: #4A4A5A; margin: 0; padding-left: 20px;">
          ${listaEstudiantes}
        </ul>
...
async function reenviarEmailFase6(id) {
  ...
  // Lógica de colores idéntica al backend (Verde si >= 10.5)
  const getColor = (nota) => (parseFloat(nota) >= 10.5 ? '#16a34a' : '#dc2626')
  const n4 = Math.round(reg.nota_fase4 || 0)
  const n5 = Math.round(reg.nota_fase5 || 0)
  const nF = Math.round(reg.nota_final_fase6 || 0)
  ...
  const previewHtml = `
    ...
    <div style="font-family: Arial, sans-serif; color: #2E004F; text-align: left; font-size: 0.85rem; border: 1px solid #E2E8F0; padding: 25px; border-radius: 12px; background-color: #FFFFFF;">
      <div style="text-align: center; margin-bottom: 20px;">
        <h3 style="color: #4B0082; margin: 0; font-size: 1.2rem;">Dictamen de Evaluación Académica</h3>
        <p style="color: #6A0DAD; font-size: 0.8rem; font-weight: bold; margin-top:5px;">Módulo 5: Experiencias Formativas (EFSRT)</p>
      </div>
      <p>Estimado/a <strong>${reg.integrante_1}</strong>,</p>
      <p>El proceso de evaluación de su proyecto ha concluido. Detalle de su calificación:</p>
      <div style="background-color: #F8F9FA; border-radius: 10px; padding: 15px; margin: 20px 0; border: 1px solid #6A0DAD;">
        <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem;">
          <tr>
            <td style="padding: 6px 0;">Nota Trabajo de Investigación (60%)</td>
            <td style="padding: 6px 0; text-align: right; font-weight: bold; color: ${getColor(reg.nota_fase4)};">${n4}</td>
          </tr>
          <tr>
            <td style="padding: 6px 0;">Nota Exposición / Defensa (40%)</td>
            <td style="padding: 6px 0; text-align: right; font-weight: bold; color: ${getColor(reg.nota_fase5)};">${n5}</td>
          </tr>
          <tr style="border-top: 2px solid #6A0DAD;">
            <td style="padding: 10px 0 0 0; font-size: 1rem; font-weight: bold; color: #4B0082;">PROMEDIO DEFINITIVO</td>
            <td style="padding: 10px 0 0 0; text-align: right; font-size: 1.2rem; font-weight: 800; color: ${getColor(reg.nota_final_fase6)};">${nF}</td>
          </tr>
        </table>
      </div>
      <p style="color: #8b7d93; font-size: 0.75rem; text-align: center; margin:0;">ID de Expediente: <strong>${reg.id_pedido}</strong></p>
    </div>`
...
async function enviarTodosFase6() {
  ...
  const listaEstudiantes = candidatos
    .map((c) => {
      // Color dinámico de nota para el resumen
      const colorNota =
        parseFloat(c.nota_final_fase6) >= 10.5 ? 'bg-success' : 'bg-danger'
      return `<li style="margin-bottom: 6px;">
        <strong>${c.integrante_1}</strong> <span class="badge ${colorNota} ms-1">${Math.round(c.nota_final_fase6)}</span><br>
        <span style="font-size: 0.75rem; color: #6A0DAD;"><i class="fas fa-envelope me-1"></i>${c.correo}</span>
      </li>`
    })
    .join('')
  const total = candidatos.length
  const htmlLista = `
    <div class="text-start">
      <p class="mb-2">Se enviará el <b>Dictamen Final</b> a <b>${total}</b> destinatarios:</p>
      <div style="max-height: 250px; overflow-y: auto; border: 1px solid #E2E8F0; border-radius: 8px; padding: 12px; background-color: #F8F9FA; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">
        <ul style="font-size: 0.85rem; color: #4A4A5A; margin: 0; padding-left: 20px;">
          ${listaEstudiantes}
        </ul>
